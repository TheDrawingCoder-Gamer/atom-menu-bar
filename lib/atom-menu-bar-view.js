  ...
  constructor(serializedState) {
    ...
    let registerIcons = (obj, iconNameSpace, menuNameSpace)  => {
      let fetchMenuTrees = function(tab) {
        let sanitizeSubMenus = function(submenu = []) {
          ...
          for (let menuOpt = 0; menuOpt < standinMenu.length; menuOpt++) {
            if (exists(standinMenu[menuOpt].label)) {
              ...
              if (exists(standinMenu[menuOpt].menu)) {
                standinMenu[menuOpt].menu = sanitizeSubMenus(standinMenu[menuOpt].menu);
              }
        ...
        registry = replaceKeysInObj(registry, "submenu", "menu");
        registry = replaceKeysInObj(registry, "command", "action");
        ...
        function sanitizeMain(obj) {
          ...
          if (obj.label.toLowerCase() === tab.toLowerCase()) {
            menuReturn = sanitizeSubMenus(obj.menu);
          }
          return obj;
        }
        registry = _.map(registry, sanitizeMain);
        //console.log(menuReturn);
        //console.log(menuReturn[0].label);
        return menuReturn;
      }
      ...
      for (var icon in obj) {
        if (obj.hasOwnProperty(icon)) {
          if (exists(obj[icon].file)) {
            Object.defineProperty(obj[icon], 'menu', {
          ...
          if (exists(obj[icon].action)) {
            console.log(obj[icon].action);
            //apparently required
            let action = obj[icon].action;
            console.log(typeof obj[icon].action);
            iconNameSpace[icon].addEventListener('click', () => {
              atom.commands.dispatch(atom.workspace.element, action);
            });
          }
          if (exists(obj[icon].menu)) {
            ...
            let registerMenus = (menu, icon1, parent, aftSep, menuNameSpace) => {
              ...
              for (let option = 0; option < menu.length; option++) {
                ...
                if (exists(menu[option].menu)) {
                  ...
                  registerMenus(menu[option].menu, icon1 + optStr, menu1, aftSep, menuNameSpace);
            ...
            registerMenus(obj[icon].menu, icon, menu, afterSeperator, menuNameSpace);
