...
const registerIcons = (obj, parent) => {
    ...
    for (var icon = 0; icon < obj.length; icon++) {
            if (exists(obj[icon].file)) {
              if (obj[icon].file === 'find' || obj[icon].file === 'packages') {
...
export default class AtomMenuBarView {
  ...
  constructor(serializedState) {
    ...
    let value = exists(serializedState) ? CSON.parseString(serializedState) : CSON.parseFile(path.join(__dirname, '..', 'obj', 'menu-bar-layout.cson'));
    var that = this;
